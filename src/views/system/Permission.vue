...
<template>
  <div>
    ...
    <el-form :inline="true" :model="formInline" class="user-search">
      ...
      <el-form-item label="">
        <el-input size="small" v-model="formInline.courseStudent" placeholder="输入学生名称"></el-input>
      </el-form-item>
      <el-form-item>
        <el-button size="small" type="primary" icon="el-icon-search" @click="search">搜索</el-button>
        <el-button size="small" type="primary" icon="el-icon-plus" @click="addForm=true">添加</el-button>
      </el-form-item>
    </el-form>
    <!--列表-->
    <el-table size="small" @selection-change="selectChange" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" style="width: 100%;">
      ...
      <el-table-column sortable prop="courseStudent" label="学生名称" width="300">
      </el-table-column>
      <el-table-column sortable prop="courseName" label="课程名称" width="300">
      </el-table-column>
      <el-table-column sortable prop="courseDrill" label="老师姓名" width="300">
      </el-table-column>
      <el-table-column sortable prop="courseAddress" label="课程地址" width="300">
      </el-table-column>
      <el-table-column align="center" label="操作" min-width="300">
        <template slot-scope="scope">
          <el-button size="mini" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
          <el-button size="mini" type="danger"  @click="deleteUser(scope.$index, scope.row)":disabled="tag==2">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @click="closeDialog">
        <el-form label-width="120px" :model="editForm"  ref="editForm">
          <el-form-item label="学生名称" prop="deptName">
            <el-input size="small" v-model="editForm.courseStudent" auto-complete="off" placeholder="请输入学生名称":disabled="tag==2"></el-input>
          </el-form-item>
          <el-form-item label="课程名称" prop="deptNo">
            <el-input size="small" v-model="editForm.courseName" auto-complete="off" placeholder="请输入课程名称"></el-input>
          </el-form-item>
          <el-form-item label="老师姓名" prop="deptName">
            <el-input size="small" v-model="editForm.courseDrill" auto-complete="off" placeholder="请输入老师姓名"></el-input>
          </el-form-item>
          <el-form-item label="课程地址" prop="deptNo">
            <el-input size="small" v-model="editForm.courseAddress" auto-complete="off" placeholder="请输入课程地址"></el-input>
          </el-form-item>
        </el-form>
...
<!--    新增界面-->
    <el-dialog :title="title" :visible.sync="addForm" width="30%" @click="closeDialog">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click="addForm=false">取消</el-button>
...
export default {
  ...
  methods: {
    ...
    getdata(parameter) {
       ...
       */
      //获取权限列表
      if(localStorage.getItem('tag')==2) {
        this.formInline.courseStudentid=localStorage.getItem('id');
        axios.post('/api/course/' + parameter.page + '/' + parameter.limit,
          ...
          .then(res => {
            ...
            if (res.success == false) {
              ...
              this.pageparm.pageSize = this.formInline.limit
              this.pageparm.total = res.count
              this.formInline.courseStudent="";
              this.formInline.courseName="";
              this.editForm.courseStudent=res.data.records[0].courseStudent;
              this.editForm.courseStudentid=localStorage.getItem('id');
            }
          })
          .catch(err => {
            this.loading = false
            this.$message.error('课程信息获取失败，请稍后再试！')
          })
    ...
    add(editForm){
      this.loading = true
      axios.post('/api/course/', JSON.stringify(this.editForm), {
        headers: {
          'Content-Type': 'application/json'
        }
      })
    ...
    submitForm(editData) {
      ...
      this.$refs[editData].validate(valid => {
        if (valid) {
          axios.put('/api/course/', JSON.stringify(this.editForm), {
            headers: {
              'Content-Type': 'application/json'
            }
          })
            ...
            .catch(err => {
              this.editFormVisible = false
              this.loading = false
              this.$message.error('课程信息修改失败，请稍后再试！')
            })
        } else {
          return false
        }
